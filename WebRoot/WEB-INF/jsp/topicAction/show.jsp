			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<td width="3" class="ForumPageTableTitleLeft">&nbsp;</td>
					...
					<td class="ForumPageTableTitle" align="right" style="padding-right:12px;">
						...
						<s:a action="topic_moveUI?id=%{#topic.id}&forumId=%{#topic.forum.id}"><img border="0" src="${pageContext.request.contextPath}/style/images/edit.gif" />移动到其他版块</s:a>
						<s:if test="#topic.type==0||#topic.type==2">
						<s:a action="topic_setBest?id=%{#topic.id}&forumId=%{#topic.forum.id}" onClick="return confirm('要把本主题设为精华吗？')"><img border="0" src="${pageContext.request.contextPath}/style/images/topicType_1.gif" />精华</s:a>
						</s:if>
						<s:if test="#topic.type==0||#topic.type==1">
						<s:a action="topic_setTop?id=%{#topic.id}&forumId=%{#topic.forum.id}" onClick="return confirm('要把本主题设为置顶吗？')"><img border="0" src="${pageContext.request.contextPath}/style/images/topicType_2.gif" />置顶</s:a>
					    </s:if>
					    <s:if test="#topic.type==1||#topic.type==2">
						<s:a action="topic_setNormal?id=%{#topic.id}&forumId=%{#topic.forum.id}" onClick="return confirm('要把本主题设为普通吗？')"><img border="0" src="${pageContext.request.contextPath}/style/images/topicType_0.gif" />普通</s:a>
					    </s:if>
					</td>
					...
					<tr>
						...
						<td align="center">
							<ul class="TopicFunc">
								<!--操作列表-->
								<li class="TopicFuncLi">
									<s:a class="detail" action="reply_delete?id=%{id}&topicId=%{#topic.id}" onClick="return confirm('确定要删除本回复吗？')"><img border="0" src="${pageContext.request.contextPath}/style/images/delete.gif" />删除</s:a>
								</li>
								<!-- 文章表情与标题 -->
								<li class="TopicSubject">
									${title}
